    ...
    try:
        ...
        for fold in folds_to_run:
            ...
            try:
                ...
                # Merge into evaluation_results.json after EVERY fold, not once at
                # the end of the whole --fold batch. Model files for a completed
                # fold are already safe on disk the moment that fold finishes, but
                # without this, a crash partway through a multi-fold run (e.g.
                # --fold 0-3 dying during fold 2) would leave folds 0-1's metrics
                # never written to the results file at all, even though their
                # models trained successfully.
                all_fold_results, avg_metrics = merge_fold_results(args.output_dir, [metrics])
                logger.info(f"evaluation_results.json updated after fold {fold+1} "
